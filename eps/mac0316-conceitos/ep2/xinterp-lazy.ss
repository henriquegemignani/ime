...
(define (add-aux a b) (numV (+ (numV-n (strict a)) (numV-n (strict b)) )))
(define (sub-aux a b) (numV (- (numV-n (strict a)) (numV-n (strict b)) )))
(define (mul-aux a b) (numV (* (numV-n (strict a)) (numV-n (strict b)) )))
(define (div-aux a b) (local ([define z (strict b)])
                        (if (equal? z (numV 0))
                            (error 'interp "division by zero")
                            (numV (/ (numV-n (strict a)) (numV-n z))))))
...
(define (auxinterp expr ds)
  (type-case CFAE expr
    ...
    [if0 (c t e) (if (equal? (strict (auxinterp c ds)) (numV 0))
                     (auxinterp t ds)
                     (auxinterp e ds))]
    ...
    [app (fun-expr args) 
         (local ([define fun-val (strict (auxinterp fun-expr ds))])
           (if (not (closureV? fun-val))
               (error 'interp "function expected")
...
(test/exn (interp (parse '{{+ 1 2} {- 2 1}})) "function expected")
(test (merge-list '(a b c) '(1)) (list (cons 'a 1)))
(ct '{with {{x 5} {z {ZOMG_THIS_IS_ERROR YEAH}}} {+ x x}} 10)
