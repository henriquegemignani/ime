...
/**                                                               **/
/** Quarto Exercocio-Programa -- Cadastro Academico               **/
/** Arquivo: cadastro.c                                           **/                                             
/**                                                               **/
...
/* Compara a string s1 e s2.
 Devolve 0 se s1 == s2, caso contrario devolve um numero positivo se o primeiro
 caracter distinto de s2 tiver valor maior que o respetivo caracter em s1.
 Devolve um numero negativo, no outro caso. 
 Similar a strcmp.
 */
...
int indiceDisciplina( char disciplina[] ) {
	...
	while( left <= right ) {
		i = (left + right) / 2;
		compara = comparaString( vet_d[ i ].sigla, disciplina );
		if( compara == 0 )
			return i;
		else if( compara < 0 ) {
			right = i - 1;
...
void matriculaAluno( int nusp, char disciplina[], FILE *fp_saida ) {
	int al = indiceAluno( nusp ), mid, compara;
	...
	for( p = vet_d[mid].inicio; p != NULL; p = p->prox_aluno ) {
		if( comparaString( vet_a[ p->ind_aluno ].nome, vet_a[ al ].nome ) < 0 )
			break;
		prev = p;
	}
	obj->prox_aluno = p;
...
void listaAlunosDaDisciplina( char disciplina[], FILE *fp_saida ) {
	...
	for( p = vet_d[mid].inicio; p != NULL; p = p->prox_aluno ) {
		fprintf( fp_saida, "\t%d %s", vet_a[p->ind_aluno].nusp, vet_a[p->ind_aluno].nome );
	}
}
...
void listaDisciplinasDoAluno( int nusp, FILE *fp_saida ) {
	...
	for( p = vet_a[al].inicio; p != NULL; p = p->prox_disc ) {
		fprintf( fp_saida, "\t%s %s", vet_d[p->ind_disc].sigla, vet_d[p->ind_disc].nome );
	}
}
